/* Train/val/test splitter — parse + presets (window.SplitLib). */
(function (global) {
  "use strict";
  ...
  function parseUpload(text, filename) {
    ...
    if (name.endsWith(".json") || String(text).trim().charAt(0) === "[" || String(text).trim().charAt(0) === "{") {
      try {
  ...
  SplitLib.MAX_ROWS = MAX_ROWS;
  SplitLib.MAX_BYTES = MAX_BYTES;
  SplitLib.isMissing = isMissing;
  SplitLib.collectColumns = collectColumns;
  SplitLib.parseUpload = parseUpload;
  SplitLib.loadPreset = loadPreset;
  SplitLib.listPresets = listPresets;
